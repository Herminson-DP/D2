              ...
              {taxes.map((tax) => (
                <tr key={tax.id} className="hover:bg-slate-50/70">
                  ...
                  <td className="p-3.5 text-center">
                    <span className="font-mono font-bold text-sm text-purple-700 bg-purple-50 px-2.5 py-1 rounded-lg">
                  ...
                  <td className="p-3.5 text-center">
                    {tax.isDefault ? (
                      ...
                      <span className="text-slate-400 text-[11px]">Específico</span>
                    )}
                  </td>

                  <td className="p-3.5 text-center">
                    <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${
                      tax.isActive ? 'bg-emerald-100 text-emerald-800' : 'bg-slate-100 text-slate-600'
                    }`}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs">
          <div className="bg-white rounded-3xl max-w-md w-full shadow-2xl border border-slate-200 p-6 space-y-4">
            ...
            <form onSubmit={handleSave} className="space-y-3">
              <div>
                <label className="block text-xs font-bold text-slate-700 mb-1">Nombre del Impuesto *</label>
                <input
                  type="text"
                  required
                  value={name}
              ...
              <div>
                <label className="block text-xs font-bold text-slate-700 mb-1">Porcentaje de Tasa (%) *</label>
                <input
                  type="number"
                  required
                  min="0"
